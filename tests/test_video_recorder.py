...
class TestVideoRecorder:
    ...
    def test_video_recorder_start_stop(self):
        ...
        # Start recording
        recorder.start_recording(str(output_path))
    ...
    def test_add_frame_rgb(self):
        ...
        recorder = VideoRecorder(fps=30, resolution=(64, 48))
    ...
    def test_add_frame_different_formats(self):
        ...
        # Test float frame (0-1 range) - convert to uint8
        float_frame = (np.random.rand(48, 64, 3) * 255).astype(np.uint8)
        ...
        # Test RGB frame (skip RGBA test as new recorder expects RGB only)
        rgb_frame = np.random.randint(0, 255, (48, 64, 3), dtype=np.uint8)
        recorder.add_frame(rgb_frame)
